...
@interface EventTypesViewController ()<AllDraftersTableViewDelegate, UITextFieldDelegate, CreateDraftVCDelegate, EditDraftVCDelegate>
{
...
@implementation EventTypesViewController
...
- (void)viewDidLoad {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self requestGetEventTypes: userInfo];
    });
}
...
- (void)saveNewEvent:(UITextField*)textField {
    if ([textField.text isEqualToString:@""]) {
        ...
        UIAlertController * alert = [UIAlertController
                                     alertControllerWithTitle:@"Event Type"
                                     message:@"New Event Type can't be empty!"
    ...
    [userInfo setObject:appController.strUserId forKey:@"user_id"];
    [userInfo setObject:appController.strToken forKey:@"token"];
    [userInfo setObject:textField.text forKey:@"event_type"];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self requestAddEventType: userInfo];
    });
}
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.lblName.text = [dicEvent objectForKey:@"event_type"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([_strNavigateFrom isEqualToString:@"create_draft"]) {
        NSDictionary *dicEventType = [arrSearchResult objectAtIndex:indexPath.row];
        [createdraftVCDelegate setEventType:dicEventType];
        [self.navigationController popViewControllerAnimated:YES];
    }
    else if ([_strNavigateFrom isEqualToString:@"edit_draft"]) {
        NSDictionary *dicEventType = [arrSearchResult objectAtIndex:indexPath.row];
        [editdraftVCDelegate setEventType:dicEventType];
        [self.navigationController popViewControllerAnimated:YES];
    }
}
...
- (void)OnHide:(int)nIndex {
    ...
    [userInfo setObject:appController.strUserId forKey:@"user_id"];
    [userInfo setObject:appController.strToken forKey:@"token"];
    [userInfo setObject:@"event_type" forKey:@"item"];
...
-(void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    UIAlertController * alert = [UIAlertController
                                 alertControllerWithTitle:@"Event Type"
                                 message:@"Are you sure to save new Event Type?"
                                 preferredStyle:UIAlertControllerStyleAlert];
...
- (IBAction)OnAdd:(id)sender {
    ...
    [dicNewEvent setObject:@"" forKey:@"event_type"];
...
- (IBAction)OnSearchChange:(id)sender {
    ...
    for (NSMutableDictionary *dicEvent in arrEvents) {
        NSString *strEventName = [dicEvent objectForKey:@"event_type"];
...
- (void) requestGetEventTypes:(id) params {
    appController.isLoading = NO;
    [commonUtils hideActivityIndicator];
    NSDictionary *resObj = nil;
    resObj = [commonUtils httpJsonRequest:API_URL_GET_EVENTTYPES withJSON:(NSMutableDictionary *) params];
    if (resObj != nil) {
...
- (void) requestAddEventType:(id) params {
    appController.isLoading = NO;
    [commonUtils hideActivityIndicator];
    NSDictionary *resObj = nil;
    resObj = [commonUtils httpJsonRequest:API_URL_CREATE_EVENTTYPE withJSON:(NSMutableDictionary *) params];
