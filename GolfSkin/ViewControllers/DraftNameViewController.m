...
@interface DraftNameViewController ()<AllDraftersTableViewDelegate, UITextFieldDelegate, CreateDraftVCDelegate>
{
...
@implementation DraftNameViewController
...
- (void)saveNewEvent:(UITextField*)textField {
    if ([textField.text isEqualToString:@""]) {
        ...
        UIAlertController * alert = [UIAlertController
                                     alertControllerWithTitle:@"Draft Name"
                                     message:@"New Draft Name can't be empty!"
    ...
    [userInfo setObject:appController.strUserId forKey:@"user_id"];
    [userInfo setObject:appController.strToken forKey:@"token"];
    [userInfo setObject:textField.text forKey:@"draft_name"];
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.lblName.text = [dicEvent objectForKey:@"draft_name"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_strNavigateFrom isEqualToString:@"create_draft"]) {
        NSDictionary *dicDraftName = [arrSearchResult objectAtIndex:indexPath.row];
        [createdraftVCDelegate setDraftName:dicDraftName];
        [self.navigationController popViewControllerAnimated:YES];
    }
    else if ([_strNavigateFrom isEqualToString:@"edit_draft"]) {
        NSDictionary *dicDraftName = [arrSearchResult objectAtIndex:indexPath.row];
        [editdraftVCDelegate setDraftName:dicDraftName];
        [self.navigationController popViewControllerAnimated:YES];
    }
}
...
- (void)OnHide:(int)nIndex {
    ...
    [userInfo setObject:appController.strUserId forKey:@"user_id"];
    [userInfo setObject:appController.strToken forKey:@"token"];
    [userInfo setObject:@"draftname" forKey:@"item"];
...
-(void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    UIAlertController * alert = [UIAlertController
                                 alertControllerWithTitle:@"Draft Name"
                                 message:@"Are you sure to save new Draft Name?"
                                 preferredStyle:UIAlertControllerStyleAlert];
...
- (IBAction)OnAdd:(id)sender {
    ...
    [dicNewEvent setObject:@"" forKey:@"draft_name"];
...
- (IBAction)OnSearchChange:(id)sender {
    ...
    for (NSMutableDictionary *dicEvent in arrEvents) {
        NSString *strEventName = [dicEvent objectForKey:@"draft_name"];
...
- (void) requestGetDraftees:(id) params {
    ...
    resObj = [commonUtils httpJsonRequest:API_URL_GET_DRAFTNAME withJSON:(NSMutableDictionary *) params];
    if (resObj != nil) {
...
- (void) requestAddDraftee:(id) params {
    ...
    [commonUtils hideActivityIndicator];
    NSDictionary *resObj = nil;
    resObj = [commonUtils httpJsonRequest:API_URL_CREATE_DRAFTNAME withJSON:(NSMutableDictionary *) params];
