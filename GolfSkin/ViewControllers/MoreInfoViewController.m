...
@interface MoreInfoViewController ()<AllDraftersTableViewDelegate, UITextFieldDelegate, CreateDraftVCDelegate, EditDraftVCDelegate>
{
...
- (void)viewDidLoad {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self requestGetMoreInfo: userInfo];
    });
}
...
- (void)saveNewEvent:(UITextField*)textField {
    if ([textField.text isEqualToString:@""]) {
        ...
        UIAlertController * alert = [UIAlertController
                                     ...
                                     message:@"New More Info can't be empty!"
    ...
    [userInfo setObject:appController.strUserId forKey:@"user_id"];
    [userInfo setObject:appController.strToken forKey:@"token"];
    [userInfo setObject:textField.text forKey:@"more_info"];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self requestAddMoreInfo: userInfo];
    });
}
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.lblName.text = [dicEvent objectForKey:@"more_info"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([_strNavigateFrom isEqualToString:@"create_draft"]) {
        NSDictionary *dicMoreInfo = [arrSearchResult objectAtIndex:indexPath.row];
        [createdraftVCDelegate setMoreInfo:dicMoreInfo];
        [self.navigationController popViewControllerAnimated:YES];
    }
    else if ([_strNavigateFrom isEqualToString:@"edit_draft"]) {
        NSDictionary *dicMoreInfo = [arrSearchResult objectAtIndex:indexPath.row];
        [editdraftVCDelegate setMoreInfo:dicMoreInfo];
        [self.navigationController popViewControllerAnimated:YES];
    }
...
- (void)OnHide:(int)nIndex {
    ...
    [userInfo setObject:appController.strUserId forKey:@"user_id"];
    [userInfo setObject:appController.strToken forKey:@"token"];
    [userInfo setObject:@"moreinfo" forKey:@"item"];
...
-(void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    UIAlertController * alert = [UIAlertController
                                 alertControllerWithTitle:@"More Info"
                                 message:@"Are you sure to save new More Info?"
                                 preferredStyle:UIAlertControllerStyleAlert];
...
- (IBAction)OnAdd:(id)sender {
    ...
    [dicNewEvent setObject:@"" forKey:@"more_info"];
...
- (IBAction)OnSearchChange:(id)sender {
    ...
    for (NSMutableDictionary *dicEvent in arrEvents) {
        NSString *strEventName = [dicEvent objectForKey:@"more_info"];
...
- (void) requestGetMoreInfo:(id) params {
    appController.isLoading = NO;
    [commonUtils hideActivityIndicator];
    NSDictionary *resObj = nil;
    resObj = [commonUtils httpJsonRequest:API_URL_GET_MOREINFO withJSON:(NSMutableDictionary *) params];
    if (resObj != nil) {
...
- (void) requestAddMoreInfo:(id) params {
    appController.isLoading = NO;
    [commonUtils hideActivityIndicator];
    NSDictionary *resObj = nil;
    resObj = [commonUtils httpJsonRequest:API_URL_CREATE_MOREINFO withJSON:(NSMutableDictionary *) params];
